...
"$PYTHON_VERSION" -m pip install finder-sidebar-editor
...
if [[ -e "$PYTHON_VIRTUAL_ENVIRONMENT"/"$PYTHON_PROJECT" ]]
then
	rm -rf "$PYTHON_VIRTUAL_ENVIRONMENT"/"$PYTHON_PROJECT"
else
	:
fi
