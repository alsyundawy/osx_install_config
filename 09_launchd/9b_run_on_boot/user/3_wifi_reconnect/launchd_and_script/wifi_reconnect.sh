...
### variables
SERVICE_NAME=com.wifi_reconnect.set
SCRIPT_INSTALL_NAME=wifi_reconnect
...
wifi_reconnect() {
    ...
    if ifconfig $INTERFACE | grep -q "status: active"; then
      ...
      if [ -z "$ip_addr" ]; then
        echo "$INTERFACE ist aktiv, hat aber keine IP"
      else
        echo "$INTERFACE ist verbunden mit IP $ip_addr"
        exit 0
      fi
    else
      echo "$INTERFACE ist nicht aktiv"
    fi
    
    # Falls nicht verbunden, dann Verbindung neu versuchen
    echo "Verbindung nicht aktiv, versuche reconnect..."
    networksetup -setairportpower $INTERFACE off
    sleep 3
    networksetup -setairportpower $INTERFACE on
    sleep 3
    #networksetup -setairportnetwork $INTERFACE "tom's_wifi"
